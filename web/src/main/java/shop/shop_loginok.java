...
public class shop_loginok extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String spw = request.getParameter("spw"); //사용자 입력한 사항
		...
		try {
			...
			sql = "select sid,spw,snm from shop_member where sid=?"; //preparestatement
			this.ps = this.con.prepareStatement(sql);
			this.ps.setString(1, sid); //무조건 1번 부터 들어감
			...
			if(id.equals("") || nm.equals("")) {	//조회된 내용이 없을 경우
				...
				if(spw.equals(pw)) {	//사용자가 입력한 패스워드와 DB에 저장된 패스워드
