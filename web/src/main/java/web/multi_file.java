...
@MultipartConfig(
	...
	//maxFileSize = 1024 * 1024 * 5, //해당 맥스 파일 용량이 5MB이상 될경우 Server Down 되어버림			
...
public class multi_file extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//name을 따로 설정하지 않으며, 배열 클래스를 이용하여  IO 값을 핸들링하게 됨
			...
			for(Part f : p) {
				...
				if(filename != null || size < 2097152) {	//파일명이 있을 경우 + 2MB 이하
